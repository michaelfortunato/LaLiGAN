...
from laliegan.data_utils.double_bump import get_double_bump_data
from laliegan.data_utils.lotka import get_lv_data
from laliegan.data_utils.pendulum import (
    get_low_dim_pendulum_data,
    get_pendulum_data,
)
...
class ReactionDiffusionDataset(Dataset):
    def __init__(
        self,
        path=f'{data_path}/reaction_diffusion.mat',
        random=False,
        mode='train',
        downsample=False,
    ):
        ...
        data['uf'] += 1e-6 * np.random.randn(
            data['uf'].shape[0], data['uf'].shape[1], data['uf'].shape[2]
        )
        data['duf'] += 1e-6 * np.random.randn(
            data['duf'].shape[0], data['duf'].shape[1], data['duf'].shape[2]
        )

        if not random:
            # consecutive samples
            training_samples = np.arange(int(0.8 * n_samples))
            val_samples = np.arange(int(0.8 * n_samples), int(0.9 * n_samples))
            test_samples = np.arange(int(0.9 * n_samples), n_samples)
        else:
            # random samples
            perm = np.random.permutation(int(0.9 * n_samples))
            training_samples = perm[: int(0.8 * n_samples)]
            val_samples = perm[int(0.8 * n_samples) :]
            test_samples = np.arange(int(0.9 * n_samples), n_samples)
            ...
            self.data.append(
                {
                    'x': torch.FloatTensor(
                        np.transpose(
                            data['uf'][:, :, samples[i - n_timesteps : i]],
                            axes=(2, 0, 1),
                        ).reshape((n_timesteps, -1))
                    ),
                    'dx': torch.FloatTensor(
                        np.transpose(
                            data['duf'][:, :, samples[i - n_timesteps : i]],
                            axes=(2, 0, 1),
                        ).reshape((n_timesteps, -1))
                    ),
                }
...
class RotatingShelfDataset(Dataset):
    def __init__(
        self, path=f'{data_path}/rotobj', mode='train', flatten=False
    ):
...
class PendulumDataset(Dataset):
    def __init__(
        self, path=f'{data_path}/pendulum', n_timesteps=2, mode='train'
    ):
        super().__init__()
        try:
        ...
        for i in range(n_ics):
            for j in range(n_steps - n_timesteps):
                self.x.append(
                    x[i, j : j + n_timesteps, :].reshape(
                        (n_timesteps, input_dim)
                    )
                )
                self.dx.append(
                    dx[i, j : j + n_timesteps, :].reshape(
                        (n_timesteps, input_dim)
                    )
                )
                self.ddx.append(
                    ddx[i, j : j + n_timesteps, :].reshape(
                        (n_timesteps, input_dim)
                    )
                )
    ...
    def __getitem__(self, idx):
        return (
            torch.cat([self.x[idx], self.dx[idx]], dim=-1),
            torch.cat([self.dx[idx], self.ddx[idx]], dim=-1),
            torch.cat([self.dx[idx], self.ddx[idx]], dim=-1),
        )
...
class LowDimPendulumDataset(Dataset):
    def __init__(
        ...
        try:
            ...
            print(
                f'Load data failed. Generating low-dim pendulum {mode} data...'
            )
            n_ics = 200 if mode == 'train' else 20
...
class LotkaVolterraDataset(Dataset):
    def __init__(
        self, path=f'{data_path}/lv', n_timesteps=2, interval=100, mode='train'
    ):
        super().__init__()
        try:
            ...
            print(
                f'Load data failed. Generating Lotka-Volterra {mode} data...'
            )
            n_ics = 200 if mode == 'train' else 20
        ...
        for i in range(n_ics):
            for j in range(n_steps - n_timesteps * interval):
                self.x.append(
                    x[i, j : j + n_timesteps * interval : interval, :].reshape(
                        (n_timesteps, input_dim)
                    )
                )
                self.dx.append(
                    dx[
                        i, j : j + n_timesteps * interval : interval, :
                    ].reshape((n_timesteps, input_dim))
                )
...
class DoubleBump(torch.utils.data.Dataset):
    def __init__(self, path=f'{data_path}/doublebump', mode='train'):
        ...
        try:
            ...
            x, d = get_double_bump_data(
                n_samples=10000 if mode == 'train' else 1000
            )
